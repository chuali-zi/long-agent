...
要在 Web 里逐个真实测试 RealOps benchmark，完整步骤与判分说明见 **[`benchmarks/WEB_MANUAL_TEST.md`](benchmarks/WEB_MANUAL_TEST.md)**（中文操作手册）。简要流程如下（**布置、验收、清场均需 `sudo`**）：
...
与 **全自动回归**（`sudo bash benchmarks/run-suite.sh ...`，见 `FINAL_LOONGARCH_CHECKLIST.md` §7）不同，Web 路径需自己在页面提问。三个一键脚本：`setup-all.sh` 布置、`verify-all.sh` 验收、`teardown-all.sh` 拆除。
