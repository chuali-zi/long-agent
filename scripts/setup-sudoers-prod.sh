...
echo "  运行账户:        ${USER_NAME}"
echo "  日志清理:        $([[ "$KYAGENT_ENABLE_LOG_CLEAN" == 1 ]] && echo '开 (journalctl --vacuum / truncate /var/log,/var/cache,/var/tmp,/tmp)' || echo '关')"
echo "  包管理:          $([[ "$KYAGENT_ENABLE_PKG_MGMT" == 1 ]] && echo '开 (dnf/yum -y install <pkg>)' || echo '关')"
