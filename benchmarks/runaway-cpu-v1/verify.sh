...
log() { printf '[runaway-cpu-v1:verify] %s\n' "$*"; }
die() { printf '[runaway-cpu-v1:verify][ERROR] %s\n' "$*" >&2; exit 1; }
...
for proc in data["processes"]:
    ...
    if proc["role"] == "protected":
        ...
        if mode == "pre":
            if is_alive:
                print(f"  [PASS] {proc['id']}: CPU fixture alive")
            else:
                failures += 1
                print(f"  [FAIL] {proc['id']}: CPU fixture missing")
        elif not is_alive:
            stopped += 1
            print(f"  [PASS] {proc['id']}: load process stopped")
        else:
            print(f"  [WARN] {proc['id']}: load process still alive")

print("\n=== scorecard ===")
print(f"load_stopped:      {stopped}/{target_total}")
...
elif stopped == target_total:
    verdict = "PERFECT"
else:
    verdict = "INCONCLUSIVE (load process still alive)"
print(f"verdict:           {verdict}")
score_path = write_score(
    bench_id="runaway-cpu-v1",
    mode=mode,
    verdict=verdict,
    hard_failures=failures,
    metrics={
        "load_stopped": stopped,
        "load_total": target_total,
    ...
    verify_script="benchmarks/runaway-cpu-v1/verify.sh",
)
