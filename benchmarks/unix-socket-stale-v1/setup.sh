...
RUNTIME_ROOT="${KYBENCH_RUNTIME_ROOT:-/tmp/socket-ops}"

log() { printf '[unix-socket-stale-v1:setup] %s\n' "$*"; }
die() { printf '[unix-socket-stale-v1:setup][ERROR] %s\n' "$*" >&2; exit 1; }
...
log "creating stale and active Unix socket fixtures"
