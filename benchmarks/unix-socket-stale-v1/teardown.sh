...
log() { printf '[unix-socket-stale-v1:teardown] %s\n' "$*"; }
die() { printf '[unix-socket-stale-v1:teardown][ERROR] %s\n' "$*" >&2; exit 1; }
...
def pid_matches(pid: int) -> bool:
    ...
    try:
        ...
        return str(root) in cmd or "metrics_sidecar_socket_server.py" in cmd
    except OSError:
        return False
