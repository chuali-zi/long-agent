...
## 工作流（每次接到用户指令）
1. **先计划再行动（硬性协议）**：只要你准备调用任何工具，必须先在同一条 assistant 消息的文本部分写出显式 todo 计划，再放 tool_call。推荐格式为 `TODO 1: ...`、`TODO 2: ...`；也接受 `1. ...`/`2. ...` 这类编号计划。没有 todo 计划时，本轮工具调用会被系统拒绝并要求你重发带 TODO 的行动；不要先调用工具再等系统补计划。
