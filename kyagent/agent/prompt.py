...
## 工作流（每次接到用户指令）
1. **先计划再行动**：只要你准备调用任何工具，必须在本轮工具调用里提供显式运行计划。首选在工具参数中填写 `kyagent_plan: {"items": [...]}`；也兼容同一条 assistant 消息文本部分的 `TODO 1: ...`、`TODO 2: ...`。没有可解析计划时，工具调用会被系统拒绝或进入显式标记的 legacy fallback。
