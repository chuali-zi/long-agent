...
- **现状（诚实声明）**：性能核心指标 bench **整体未达标**（`overall_pass=false`），最新复测中 `ask` p50 / p95 未达标，`guardrail` 与 `audit` 路径仍明显快于基线。详见《05-软件性能测试报告》。
...
| 需求类别 | 验证方式 | 证据 |
| --- | --- | --- |
| 功能性需求 FR-1~FR-6 | pytest 单元/集成测试 + RealOps 9 个端到端 bench | `tests/`、`benchmarks/`（pytest 831 passed / 3 skipped；9 bench 全部 PERFECT） |
