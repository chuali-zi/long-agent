...
赛题的核心挑战是 AI 推理的不可控性。一种常见做法是让 LLM 直接生成 shell 命令，再用正则在事后过滤危险命令；这种方式需要不断追赶新出现的绕过写法。kyagent 采用另一种思路——让危险命令在源头就无法被构造出来：

```text
LLM 只能输出： tool_use(name="process_list", input={"sort_by":"cpu"})   ← 不是 shell 字符串
        │
        ▼  Tool.validate()      JSON Schema 校验参数（类型/枚举/正则/边界），拒绝未知字段
        ▼  Tool.build_argv()    由「代码」构造 argv 列表（含 `--` 终止符、禁 shell 元字符）
        │                       例：["ps","-eo","...","--sort","-pcpu"]   ← list[str]，非拼接
        ▼  subprocess.Popen(argv, shell=False)    从不 shell=True，直接 execve
        ▼  sudoers 精确白名单   显式拉黑 /bin/sh /bin/bash python perl awk sed
```
...
| 不变量                                                             | 落地位置                                     |
| --------------------------------------------------------------- | ---------------------------------------- |
| LLM 输出只有结构化 `tool_use`，绝不把文本当命令执行                               | `kyagent/agent/core.py`                  |
| argv 由 `build_argv()` 代码构造，参数先过 JSON Schema                     | `kyagent/mcp/tools/base.py`              |
| `build_argv` 显式拒绝 shell 元字符 `;                                  | & $ 换行`、路径黑名单、`--` 终止符                   |
| 执行层**从不使用 `shell=True`**，干净 env 去 `LD_PRELOAD`                  | `kyagent/executor/proxy.py`、`sandbox.py` |
| 采样型工具把「间隔/次数」编进命令本身（`iostat -dx 1 2`），绝不 `sh -c "...; sleep N"` | `kyagent/mcp/tools/base.py`（`TrendTool`） |
| sudoers 仅放行绝对路径白名单命令，显式拉黑所有 shell/解释器                           | `configs/sudoers.kyagent`                |
...
| FR-3.2 | 工具层 `build_argv` 显式禁止 shell 元字符（`;                                                   | & $ 换行`）、限定子命令枚举、路径黑名单                                                             |
...
- 实现：`IntentGuard` 对用户原始自然语言做注入识别；读取不可信系统文件（如 `/etc/cron.d` 中含敌意文本的脚本）时，将内容视为**证据**而非指令。
